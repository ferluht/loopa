...
void DAW::draw(GFXcanvas1 *screen) {
    if (last_midi_time > (SAMPLERATE / AUDIO_BUF_SIZE) * 60 && reloaded == false) {
        std::exit(0);
    }
    AMG::draw(screen);
    obd.draw(screen);
    if (kbd.isactive) kbd.draw(screen);
}
...
//
//        auto selected_instrument = getCurrentTrackInstrument();
//        screen->setTextSize(1);
//        auto instr_name = selected_instrument->getName();
//        int16_t x1, y1;
//        uint16_t w, h;
...
//
//        selected_instrument->draw(screen);
//
...
//    });
    ...
    addDrawHandler({SCREENS::TAPE_VIEW}, [this](GFXcanvas1 * screen) -> void {
        ...
        screen->setTextSize(1);
        auto s = "TRACK " + std::to_string(tracks->get_focus_index()) + ": " + selected_instrument->getName();
...
//        get_ssid(ssid);
//        get_pwd(pwd);
//        screen->setCursor(40, 16);
//        screen->print(("WIFI SSID: " + ssid).c_str());
//        screen->setCursor(40, 24);
//        screen->print(("WIFI PASS: " + pwd).c_str());
//        if (simulated_cfg) {
//            screen->setCursor(120, 24);
//            screen->print("S");
//        }
//
//
