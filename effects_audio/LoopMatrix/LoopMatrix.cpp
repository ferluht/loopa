...
LoopMatrix::LoopMatrix() : AudioEffect("TAPES") {
    ...
    addMIDIHandler(MIDI::GENERAL::CC_HEADER, MIDI::GENERAL::CC_HEADER + m * n,
                   ...
                   [this](MData &cmd) -> MIDISTATUS {
                       int scene_idx = cmd.status - MIDI::GENERAL::CC_HEADER;
...
void LoopMatrix::draw(GFXcanvas1 *screen)  {
    switch (current_screen) {
        case (0):
            draw_main_screen(screen);
            break;
        case (1):
            draw_fx_screen(screen);
            break;
        default:
            break;
    }
...
void LoopMatrix::draw_main_screen(GFXcanvas1 *screen) {
    for (int i = 0; i < 4; i ++) {
